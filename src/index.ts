 ...
 * current applicable date. So hnb_currency_rate fetches the range across ALL currencies and filters
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Croatian National Bank');
}
...
async function hnbGet(params: URLSearchParams): Promise<unknown> {
  ...
  const res = await pwFetch(qs ? `${BASE}?${qs}` : BASE, {
